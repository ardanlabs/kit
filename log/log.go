...
func (l *Logger) Dev(ctx interface{}, funcName string, format string, a ...interface{}) {
	l.mu.RLock()
	{
		if l.level() == DEV {
			...
			l.Output(mLevel, fmt.Sprintf("DEV : %s : %s : %s", ctx, funcName, format))
		}
	}
	l.mu.RUnlock()
}
...
func (l *Logger) User(ctx interface{}, funcName string, format string, a ...interface{}) {
	l.mu.RLock()
	{
		if l.level() >= DEV {
			...
			l.Output(mLevel, fmt.Sprintf("USER : %s : %s : %s", ctx, funcName, format))
		}
	}
	l.mu.RUnlock()
}
...
func (l *Logger) Error(ctx interface{}, funcName string, err error, format string, a ...interface{}) {
	l.mu.RLock()
	{
		if l.level() >= DEV {
			...
			l.Output(mLevel, fmt.Sprintf("ERROR : %s : %s : %s : %s", ctx, funcName, err, format))
		}
	}
	l.mu.RUnlock()
}
...
func (l *Logger) Fatal(ctx interface{}, funcName string, format string, a ...interface{}) {
	l.mu.RLock()
	{
		if l.level() >= DEV {
			...
			l.Output(mLevel, fmt.Sprintf("FATAL : %s : %s : %s", ctx, funcName, format))
		}
	}
	l.mu.RUnlock()
...
func (l *Logger) DevOffset(ctx interface{}, offset int, funcName string, format string, a ...interface{}) {
	l.mu.RLock()
	{
		if l.level() == DEV {
			...
			l.Output(mLevel+offset, fmt.Sprintf("DEV : %s : %s : %s", ctx, funcName, format))
		}
	}
	l.mu.RUnlock()
}
...
func (l *Logger) UserOffset(ctx interface{}, offset int, funcName string, format string, a ...interface{}) {
	l.mu.RLock()
	{
		if l.level() >= DEV {
			...
			l.Output(mLevel+offset, fmt.Sprintf("USER : %s : %s : %s", ctx, funcName, format))
		}
	}
	l.mu.RUnlock()
}
...
func (l *Logger) ErrorOffset(ctx interface{}, offset int, funcName string, err error, format string, a ...interface{}) {
	l.mu.RLock()
	{
		if l.level() >= DEV {
			...
			l.Output(mLevel+offset, fmt.Sprintf("ERROR : %s : %s : %s : %s", ctx, funcName, err, format))
		}
	}
	l.mu.RUnlock()
}
...
func (l *Logger) FatalOffset(ctx interface{}, offset int, funcName string, format string, a ...interface{}) {
	l.mu.RLock()
	{
		if l.level() >= DEV {
			...
			l.Output(mLevel+offset, fmt.Sprintf("FATAL : %s : %s : %s", ctx, funcName, format))
		}
	}
	l.mu.RUnlock()
